...
if ($orders) {
    $delete_orders = mysqli_query($db, "DELETE FROM cheques WHERE id_cheque IN (" . implode(',', $orders) . ")");
}
...
<body>
    ...
    <section class="section_admin_orders">
        <div class="container">
            <form action='/admin-orders-delete.php' method='POST'>
                <?php
                ...
                if (mysqli_num_rows($active_orders) > 0) {
                    ...
                    ?>
                    <button style="margin-top:25px;" type='submit'>Delete Orders</button>
                <?
                } else {
                    echo "No orders☹️";
                }
